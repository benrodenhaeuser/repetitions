...
- idea:
  - we need a data structure that records elements we have already seen as we
    traverse the array from left to right. => hash

- algo:
  - initialize empty hash `seen`
  - iterate over given array:
    - given elem on current iteration:
      if seen[elem]:
        return elem
      else:
        seen[elem] = true
...
def find_dup(array)
  seen = {}
  array.each { |elem| seen[elem] ? (break elem) : seen[elem] = true }
end
